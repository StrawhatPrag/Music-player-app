...
const signup = async (req, res) => {
  try {
    ...
    if (!name || !email || !password) {
      return res
        .status(400)
        .json({ message: "Name, email id and password is required" });
    }
...
const login = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({ message: "Email id doesnt exists" });
    }
...
const editProfile = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: error?.message || "Error in updating profile" });
  }
};
